...
- **`AnthropicProvider`** — Subprocess wrapper around the locally-installed `claude` CLI. Model pinned to `claude-sonnet-4-6` via `--model sonnet` (override with `CLAUDE_CLI_MODEL` env var). OAuth keychain auth on server — no API key. All structured methods use `--json-schema` for strict output (CLI enforces via internal tool-calling; result is parsed from the JSON envelope's `structured_output` field). Streaming uses `--output-format stream-json --include-partial-messages --verbose`. Module-level `BoundedSemaphore` caps concurrent subprocesses (`CLAUDE_CLI_MAX_CONCURRENCY=5` default). `_CLI_HEALTHY` flag toggled by app startup probe + 5-min ticker; `_can_use_claude` in `app.py` is the single gate-site source of truth (returns `is_developer or plan_tier=='pro'` ∧ `_CLI_HEALTHY`). Ops kill switch: `CLAUDE_CLI_DISABLED=1` forces unhealthy (Pro users transparently fall back to Ollama).
...
16. **Claude CLI cwd**: The CLI auto-discovers `CLAUDE.md` in `cwd` and prepends it to system prompts. `_run_cli` / `_run_cli_stream` in `pipeline/providers.py` MUST run with `cwd=/tmp/jobapp-claude/` (a dir intentionally containing no CLAUDE.md) AND pass `--exclude-dynamic-system-prompt-sections`. Both belt and suspenders — drop either and the project CLAUDE.md leaks into every Claude prompt, polluting outputs and burning ~1500 tokens of input on every call.
